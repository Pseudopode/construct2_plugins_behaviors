if (window.CanvasRenderingContext2D && CanvasRenderingContext2D.prototype.getImageData){
	...
	CanvasRenderingContext2D.prototype.blendFrom = function(srcContext,blendMode,offsetOptions){
	//srcContext = otherContext, destContext = me
		...
		offsets.width  = Math.min(offsets.width, srcContext.canvas.width-offsets.sourceX, this.canvas.width-offsets.destX );
		offsets.height = Math.min(offsets.height,srcContext.canvas.height-offsets.sourceY,this.canvas.height-offsets.destY);
		...
		var dstD = this.getImageData(offsets.destX,offsets.destY,offsets.width,offsets.height);
		var src  = srcD.data;
		var dst  = dstD.data;
		...
		this.putImageData(dstD,offsets.destX,offsets.destY);
	};
	...
	var modes = CanvasRenderingContext2D.prototype.blendFrom.supportedBlendModes = 'normal src-over screen multiply difference src-in plus add overlay hardlight colordodge dodge colorburn burn darken lighten exclusion'.split(' ');
	//var supports = CanvasRenderingContext2D.prototype.blendOnto.supports = {};
	var supports = CanvasRenderingContext2D.prototype.blendFrom.supports = {};
